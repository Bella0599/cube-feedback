...
if st.button("✨ 큐브어학원 프리미엄 피드백 생성"):
    if not selected_student:
        ...
        if is_phonics and test_range == "Final Test (Unit 1-8)":
            ...
            primary_narrative = f"이번 파닉스 Final Test(Unit 1-8)를 통해 종합적으로 점검한 결과, 1단원부터 8단원까지의 전체적인 음가와 규칙을 훌륭하게 마스터하고 뛰어난 언어적 이해도를 보여주었습니다. 성실하게 전체 단원을 마무리한 {selected_en_name}를 크게 칭찬해 주고 싶습니다! "
            if bad_str:
                primary_narrative += f" {bad_str}영역의 발음 및 규칙 적용 부분을 조금 더 신경 써서 섬세하게 보완하겠습니다."
            else:
                primary_narrative += "모든 영역에서 뛰어난 모습을 보여주었으며, 이 단단한 기초를 바탕으로 다음 레벨에서도 흔들림 없이 훌륭한 모습을 이어갈 것이라 확신합니다."
        else:
            if p_score_num >= 90:
