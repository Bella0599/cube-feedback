...
try:
    ...
    obj_df = load_objective_data(sheet_url)
    
    OBJECTIVE_DB = {}
    for _, row in obj_df.iterrows():
        b_name = str(row['교재명']).strip()
        u_name = str(row['단원']).strip()
        obj_text = str(row['학습목표']).strip()
        if b_name not in OBJECTIVE_DB:
            OBJECTIVE_DB[b_name] = {}
        OBJECTIVE_DB[b_name][u_name] = obj_text

except:
    st.error("구글 시트 연결을 기다리고 있습니다. 시트 주소 및 탭 이름(학생명단, 교재정보)을 확인해 주세요.")
    st.stop()
...
all_books_list = list(OBJECTIVE_DB.keys()) if OBJECTIVE_DB else ["구글 시트에 등록된 교재가 없습니다."]

st.markdown("🔺 **[Main Book 설정]**")
col_m1, col_m2, col_m3 = st.columns([2, 1, 1])
with col_m1: main_book = st.selectbox("교재 선택", all_books_list, key="m_book")
with col_m2: 
    available_main_units = list(OBJECTIVE_DB.get(main_book, {}).keys()) if main_book in OBJECTIVE_DB else UNITS
    main_unit_single = st.selectbox("집중 평가 단원", available_main_units, key="m_unit")
with col_m3: main_score = st.text_input("점수", placeholder="예: 90", key="m_score")

main_objective = OBJECTIVE_DB.get(main_book, {}).get(main_unit_single, DEFAULT_OBJECTIVE) if OBJECTIVE_DB else DEFAULT_OBJECTIVE
...
with col_s1: sub_book = st.selectbox("교재 선택", ["선택안함"] + all_books_list, key="s_book")
with col_s2: 
    available_sub_units = list(OBJECTIVE_DB.get(sub_book, {}).keys()) if sub_book in OBJECTIVE_DB else UNITS
    sub_unit_single = st.selectbox("집중 평가 단원", available_sub_units, key="s_unit") if sub_book != "선택안함" else None
with col_s3: sub_score = st.text_input("점수", placeholder="예: 85", key="s_score") if sub_book != "선택안함" else ""

if sub_book and sub_book != "선택안함":
    sub_objective = OBJECTIVE_DB.get(sub_book, {}).get(sub_unit_single, DEFAULT_OBJECTIVE) if OBJECTIVE_DB else DEFAULT_OBJECTIVE
...
analysis_areas = []
if main_book and "Phonics" in main_book: analysis_areas.append("Phonics (파닉스)")
else: analysis_areas.append("Reading (독해/리딩)")
if sub_book and sub_book != "선택안함":
    if "Grammar" in sub_book: analysis_areas.append("Grammar (문법)")
    elif "Writing" in sub_book: analysis_areas.append("Writing (영작)")
if not analysis_areas: analysis_areas = ["Reading (독해/리딩)"]

selected_weaknesses = {}
for area in analysis_areas:
    if area in DIAGNOSIS_DB:
...
# 💡 [원장님 요청 적용] 5. 담당 강사 개별 소견 (템플릿 12종 적용)
...
if st.button("✨ 큐브어학원 프리미엄 피드백 생성"):
    if not selected_student:
        ...
        # 💡 [핵심] 코멘트 템플릿 + 개별 코멘트 조립
        custom_processed_text = ""
        
        # 1. 템플릿 선택 시 (학생 이름 자동 적용)
...
if st.session_state.generated_feedback:
    st.divider()
    st.text_area("📋 완성된 피드백", value=st.session_state.generated_feedback, height=700)
