...
        st.session_state.generated_feedback = f"""
        ...
        # --- 5. 이번 달 유닛별 성취 유형 및 상세 분석 ---
# (코드 중간 5번 항목의 부교재 로직 부분을 아래와 같이 수정하세요)

    # 부교재 맞춤형 선택 로직 (점수 입력이 없어도 작동하도록 수정)
    if not is_phonics and sub_book and sub_book != "선택안함":
        st.markdown("---")
        # 점수가 입력되었을 때만 점수 기반 평가, 입력 안 되면 기본 멘트 출력
        if sub_score and sub_score.strip():
            s_score_num = parse_score(sub_score)
            if s_score_num >= 90:
                tier = "high"
            elif s_score_num >= 75:
            ...
            sub_opt = st.radio(f"부교재({sub_book}) 성취도 코멘트 선택", list(SUB_BOOK_NARRATIVE_DB[tier].keys()))
            selected_sub_narrative = SUB_BOOK_NARRATIVE_DB[tier][sub_opt]
        else:
            st.info("ℹ️ 부교재 점수가 입력되지 않아 일반 격려 멘트로 대체됩니다.")
            selected_sub_narrative = "부교재 과정 또한 수업에 성실하게 참여하며 배운 내용을 차근차근 잘 소화하고 있습니다. 앞으로도 이 흐름을 이어가며 꾸준히 학습할 수 있도록 격려하겠습니다."
안녕하세요, 큐브어학원입니다.
...
    st.divider()
    st.text_area("📋 완성된 피드백 (마우스로 클릭하여 전체 복사 가능)", value=st.session_state.generated_feedback, height=800) 여기에 부교재 점수 안써도 되게 바꿔
